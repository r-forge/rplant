RunMuscle <- function(user.name, token, DE.file.name, DE.file.path="", job.name=NULL, tnrs=FALSE) {
  ...
  if (is.na(tryCatch(DE.file.name[[1]][1], error=function(x){x <- NA; return(x)}))){
    JobNum <- SubmitJob(user.name, token, application="muscle-ranger-2.0",
                        DE.file.name=quote, DE.file.path="",
                        job.name=job.name, nprocs="1")}else{
  ...
  # }
    JobNum <- SubmitJob(user.name, token, application="muscle-ranger-2.0",
                        DE.file.name=quote, DE.file.path=DE.file.path,
