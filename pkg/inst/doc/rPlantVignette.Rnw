...
<<inputres, echo=FALSE, eval=TRUE, results=verbatim>>=
require(rPlant)
# username <- "bleplant"
...
DeleteJob(ALL=TRUE)
tryCatch(UploadFile(local.file.name="DNA.fasta", filetype="FASTA-0"), error=function(err){return(paste(err))})
@
...
<<uploadfileproteinres, echo=FALSE, eval=TRUE, results=hide>>=
data(PROTEIN.fasta)
write.fasta(sequences=PROTEIN.fasta, names=names(PROTEIN.fasta), file.out="PROTEIN.fasta")
