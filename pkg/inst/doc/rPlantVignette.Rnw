...
\noindent If you don't want a job to run any more, then simply stop it by using the {\ttfamily KillJob} function. \\
...
tryCatch(UploadFile(local.file.name="simulation1.map"), error=function(err){return(paste(err))})
tryCatch(UploadFile(local.file.name="simulation1.ped"), error=function(err){return(paste(err))})
...
DeleteDir("hello")
DeleteFile("DNA.fasta")
DeleteFile("PROTEIN.fasta")
DeleteFile("geno_test.tfam")
DeleteFile("geno_test.tped")
DeleteFile("simulation1.map")
DeleteFile("simulation1.ped")
DeleteJob(ALL = TRUE)
@
