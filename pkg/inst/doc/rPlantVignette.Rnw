...
username <- "henryl"
password <- "Blizzard1"
tryCatch(Validate(username, password, api="foundation"), error=function(err){return(paste(err))})
tryCatch(Validate(username, password, api="foundation"), error=function(err){return(paste(err))})
@
}
...
\noindent The move took the file DNA.fasta from the home directory into the ``hello/all'' directory. \\
...
The job number is used in a few rPlant functions including, {\ttfamily CheckJobStatus}, {\ttfamily KillJob}, {\ttfamily ListJobOutput}, {\ttfamily RetrieveJob} and {\ttfamily DeleteJob}. If you need to get job IDs from older jobs, you can use the function {\ttfamily GetJobHistory()}. 
...
CheckJobStatus(myJobS$id)
@
}

\subsection{Killing Job}

\noindent If you don't want a job to run any more, then simply stop it by using the {\ttfamily KillJob} function.  Generally this is used if a job is {\ttfamily QUEUED}, it must be stopped first to delete it.
...
Wait(myJobS$id, minWait, maxWait)
@

{\fs
<<ListJobOutput, echo=TRUE, results=verbatim>>=
ListJobOutput(myJobS$id)
@
}
...
\noindent This function displays the entire job history for the user. This is an easy way to grab old job IDs in order to retrieve files or check the status of a set of jobs.  You can see that one finished and one was stopped.  
...
<<WaitAll1, results=HIDE, echo=FALSE>>=
Wait(myJobMuDP$id, minWait, maxWait)
Wait(myJobMuDF$id, minWait, maxWait)
Wait(myJobMuDPP$id, minWait, maxWait)
Wait(myJobMuDPS$id, minWait, maxWait)
Wait(myJobMuDC$id, minWait, maxWait)
Wait(myJobMuDM$id, minWait, maxWait)
Wait(myJobMuPP$id, minWait, maxWait)
Wait(myJobMuPF$id, minWait, maxWait)
Wait(myJobMuPPP$id, minWait, maxWait)
Wait(myJobMuPPS$id, minWait, maxWait)
Wait(myJobMuPC$id, minWait, maxWait)
Wait(myJobMuPM$id, minWait, maxWait)
Wait(myJobMaDF$id, minWait, maxWait)
Wait(myJobMaDC$id, minWait, maxWait)
Wait(myJobMaPF$id, minWait, maxWait)
Wait(myJobMaPC$id, minWait, maxWait)
Wait(myJobCWDP$id, minWait, maxWait)
Wait(myJobCWDC$id, minWait, maxWait)
Wait(myJobCWDN$id, minWait, maxWait)
Wait(myJobCWDGCG$id, minWait, maxWait)
Wait(myJobCWDGDE$id, minWait, maxWait)
Wait(myJobCWDPIR$id, minWait, maxWait)
Wait(myJobCWPP$id, minWait, maxWait)
Wait(myJobCWPC$id, minWait, maxWait)
Wait(myJobCWPN$id, minWait, maxWait)
Wait(myJobCWPGCG$id, minWait, maxWait)
Wait(myJobCWPGDE$id, minWait, maxWait)
Wait(myJobCWPPIR$id, minWait, maxWait)
@

\subsection{FastTree}

\noindent {\ttfamily Fasttree <- function(file.name, file.path="", job.name=NULL, args=NULL, type="DNA", model=NULL, gamma=FALSE, stat=FALSE, print.curl=FALSE, version="fasttreeDispatcher-1.0.0u1", shared.username=NULL, suppress.Warnings=FALSE)} \\

\noindent {\ttfamily FastTree} infers approximately-maximum-likelihood phylogenetic trees from alignments of nucleotide or protein sequences. See \url{http://meta.microbesonline.org/fasttree/}

{\fs
<<FasttreeDNA, echo=TRUE>>=
myJobFaDMuP <- Fasttree("phylip_interleaved.aln", file.path=paste("analyses/",myJobMuDP$name, 
                        sep=""), job.name="fasttreeMUSCLEdnaPHY")
myJobFaDCWP <- Fasttree("clustalw2.fa", file.path=paste("analyses/",myJobCWDP$name, sep=""), 
                        job.name="fasttreeCLUSTALWdnaPHY")
myJobFaDMuF <- Fasttree("fasta.aln", file.path=paste("analyses/",myJobMuDF$name, sep=""), 
                        job.name="fasttreeMUSCLEdnaFASTA")
myJobFaDCWF <- Fasttree("mafft.fa", file.path=paste("analyses/",myJobMaDF$name, sep=""), 
                        job.name="fasttreeMAFFTdnaFASTA")
@
}

{\fs
<<FasttreeProtein, echo=TRUE>>=
myJobFaPMuP <- Fasttree("phylip_interleaved.aln", file.path=paste("analyses/",myJobMuPP$name, 
                       sep=""), type="PROTEIN", 
                       job.name="fasttreeMUSCLEproteinPHY")
myJobFaPCWP <- Fasttree("clustalw2.fa", type="PROTEIN", file.path=paste("analyses/",myJobCWPP$name, 
                        sep=""), job.name="fasttreeCLUSTALWproteinPHY")
myJobFaPMuF <- Fasttree("fasta.aln", file.path=paste("analyses/",myJobMuPF$name, sep=""), 
                        type="PROTEIN", job.name="fasttreeMUSCLEproteinFASTA")
myJobFaPCWF <- Fasttree("mafft.fa", file.path=paste("analyses/",myJobMaPF$name, sep=""), 
                        type="PROTEIN", job.name="fasttreeMAFFTproteinFASTA")
@
}

\noindent {\ttfamily Fasttree} outputs trees in Newick format \url{http://en.wikipedia.org/wiki/Newick_format}. The placement of the root is not biologically meaningful. The local support values are given as names for the internal nodes, and range from 0 to 1, not from 0 to 100 or 0 to 1,000. If all sequences are unique, then the tree will be fully resolved (the root will have three children and other internal nodes will have two children). If there are multiple sequences that are identical to each other, then there will be a multifurcation. Also, there are no support values for the parent nodes of redundant sequences. 

\subsection{RAxML (Randomized Accelerated Maximum Likelihood)}

\noindent {\ttfamily RAxML(file.name, file.path="", job.name=NULL, type="DNA", model=NULL, bootstrap=NULL, algorithm="d", multipleModelFileName=NULL, args=NULL, numcat=25, nprocs=12, \\
\noindent version="raxml-lonestar-7.2.8u1", print.curl=FALSE, shared.username=NULL, \\
\noindent substitution\_matrix=NULL, empirical.frequencies=FALSE, suppress.Warnings=FALSE) } \\

\noindent {\ttfamily RAxML} is a program for sequential and parallel Maximum Likelihood based inference of large phylogenetic tress.  It has originall been derived from from fastDNAml which in turn was derived from Joe Felsentein's dnaml which is part of the PHYLIP package. See \url{http://sco.h-its.org/exelixis/oldPage/RAxML-Manual.7.0.4.pdf} for details.

{\fs
<<RAxMLDNA, echo=TRUE>>=
myJobRDMuP <- RAxML("phylip_interleaved.aln", file.path=paste("analyses/",myJobMuDP$name, 
                    sep=""), job.name="raxmlMUSCLEdnaPHY")
myJobRDCWP <- RAxML("clustalw2.fa", file.path=paste("analyses/",myJobCWDP$name, sep=""), 
                    job.name="raxmlCLUSTALWdnaPHY")
@
}

{\fs
<<RAxMLProtein, echo=TRUE>>=
myJobRPMuP <- RAxML("phylip_interleaved.aln", file.path=paste("analyses/",myJobMuPP$name, 
                    sep=""), type="PROTEIN", job.name="raxmlMUSCLEproteinPHY")
myJobRPCWP <- RAxML("clustalw2.fa", file.path=paste("analyses/",myJobCWPP$name, sep=""), 
                    type="PROTEIN", job.name="raxmlCLUSTALWproteinPHY")
@
}

\noindent For this application there are numerous output files.  See pg 16-17 of the manual for complete details.  RAxML outputs trees in Newick format \url{http://en.wikipedia.org/wiki/Newick\_format}.

\subsection{PHYLIP-Parsimony 3.69}

\noindent {\ttfamily PHYLIP\_Pars(file.name, file.path="", job.name=NULL, type="DNA", print.curl=FALSE, shared.username=NULL, suppress.Warnings=FALSE) } \\

\noindent {\ttfamily PHYLIP} is a free package of programs for inferring phylogenies. It is distributed as source code, documentation files, and a number of different types of executables. The web page: \url{http://evolution.genetics.washington.edu/phylip/doc/main.html}, by Joe Felsenstein of the Department of Genome Sciences and the Department of Biology at the University of Washington, contain information on PHYLIP.  PHYLIP (the PHYLogeny Inference Package) is a package of programs for inferring phylogenies (evolutionary trees).  Methods that are available in the package include parsimony, distance matrix, and likelihood methods, including bootstrapping and consensus trees.

{\fs
<<phylipDNA, echo=TRUE>>=
myJobPDMuPP <- PHYLIP_Pars("phylip_pars.aln", file.path=paste("analyses/",myJobMuDPP$name, 
                           sep=""), job.name="phylipMUSCLEdnaPHYpars")
@

<<phylipProtein, echo=TRUE>>=
myJobPPMuPP <- PHYLIP_Pars("phylip_pars.aln", file.path=paste("analyses/",myJobMuPPP$name, 
                           sep=""), type="PROTEIN", job.name="phylipMUSCLEproteinPHYpars")
@
}

\noindent {\ttfamily PHYLIP Parsimon}y outputs trees in Newick format \url{http://en.wikipedia.org/wiki/Newick_format}.

<<WaitAll2, results=HIDE, echo=FALSE>>=
Wait(myJobFaDMuP$id, minWait, maxWait)
Wait(myJobFaDCWP$id, minWait, maxWait)
Wait(myJobFaDMuF$id, minWait, maxWait)
Wait(myJobFaDCWF$id, minWait, maxWait)
Wait(myJobFaPMuP$id, minWait, maxWait)
Wait(myJobFaPCWP$id, minWait, maxWait)
Wait(myJobFaPMuF$id, minWait, maxWait)
Wait(myJobFaPCWF$id, minWait, maxWait)
Wait(myJobRDMuP$id, minWait, maxWait)
Wait(myJobRDCWP$id, minWait, maxWait)
Wait(myJobRDMuP$id, minWait, maxWait)
Wait(myJobRDCWP$id, minWait, maxWait)
Wait(myJobRPMuP$id, minWait, maxWait)
Wait(myJobRPCWP$id, minWait, maxWait)
Wait(myJobPDMuPP$id, minWait, maxWait)
Wait(myJobPPMuPP$id, minWait, maxWait)
Wait(myJobCWPPIR$id, minWait, maxWait)
@

<<RetrieveJobs, results=HIDE, echo=FALSE>>=
RetrieveJob(myJobMuDP$id, c("phylip_interleaved.aln"))
RetrieveJob(myJobMuDF$id, c("fasta.aln"))
RetrieveJob(myJobMuDPP$id, c("phylip_pars.aln"))
RetrieveJob(myJobMuDPS$id, c("phylip_sequential.aln"))
RetrieveJob(myJobMuDC$id, c("clustalw.aln"))
RetrieveJob(myJobMuDM$id, c("msf.aln"))
RetrieveJob(myJobMuPP$id, c("phylip_interleaved.aln"))
RetrieveJob(myJobMuPF$id, c("fasta.aln"))
RetrieveJob(myJobMuPPP$id, c("phylip_pars.aln"))
RetrieveJob(myJobMuPPS$id, c("phylip_sequential.aln"))
RetrieveJob(myJobMuPC$id, c("clustalw.aln"))
RetrieveJob(myJobMuPM$id, c("msf.aln"))
RetrieveJob(myJobMaDF$id, c("mafft.fa"))
RetrieveJob(myJobMaDC$id, c("mafft.fa"))
RetrieveJob(myJobMaPF$id, c("mafft.fa"))
RetrieveJob(myJobMaPC$id, c("mafft.fa"))
RetrieveJob(myJobCWDP$id, c("clustalw2.fa"))
RetrieveJob(myJobCWDC$id, c("clustalw2.fa"))
RetrieveJob(myJobCWDN$id, c("clustalw2.fa"))
RetrieveJob(myJobCWDGCG$id, c("clustalw2.fa"))
RetrieveJob(myJobCWDGDE$id, c("clustalw2.fa"))
RetrieveJob(myJobCWDPIR$id, c("clustalw2.fa"))
RetrieveJob(myJobCWPP$id, c("clustalw2.fa"))
RetrieveJob(myJobCWPC$id, c("clustalw2.fa"))
RetrieveJob(myJobCWPN$id, c("clustalw2.fa"))
RetrieveJob(myJobCWPGCG$id, c("clustalw2.fa"))
RetrieveJob(myJobCWPGDE$id, c("clustalw2.fa"))
RetrieveJob(myJobCWPPIR$id, c("clustalw2.fa"))
RetrieveJob(myJobFaDMuP$id, c("fasttree.nwk"))
RetrieveJob(myJobFaDCWP$id, c("fasttree.nwk"))
RetrieveJob(myJobFaDMuF$id, c("fasttree.nwk"))
RetrieveJob(myJobFaDCWF$id, c("fasttree.nwk"))
RetrieveJob(myJobFaPMuP$id, c("fasttree.nwk"))
RetrieveJob(myJobFaPCWP$id, c("fasttree.nwk"))
RetrieveJob(myJobFaPMuF$id, c("fasttree.nwk"))
RetrieveJob(myJobFaPCWF$id, c("fasttree.nwk"))
RetrieveJob(myJobRDMuP$id, c("RAxML_bestTree.nwk"))
RetrieveJob(myJobRDCWP$id, c("RAxML_bestTree.nwk"))
RetrieveJob(myJobRPMuP$id, c("RAxML_bestTree.nwk"))
RetrieveJob(myJobRPCWP$id, c("RAxML_bestTree.nwk"))
RetrieveJob(myJobPDMuPP$id, c("outtree.nwk"))
RetrieveJob(myJobPPMuPP$id, c("outtree.nwk"))
@

\subsection{Genome Wide Association Study models}

\noindent Upload the sample files.  They are in the transposed PLINK format \url{http://pngu.mgh.harvard.edu/~purcell/plink/data.shtml#tr}.

{\fs
<<uploadfilegwas, echo=TRUE, eval=FALSE>>=
data(geno_test.tfam)
...
<<uploadfilegwasres, echo=FALSE, eval=TRUE, results=verbatim>>=
data(geno_test.tfam)
write.table(geno_test.tfam, file = "geno_test.tfam", row.names=FALSE, 
            col.names=FALSE, quote=FALSE, sep="\t")
tryCatch(UploadFile(local.file.name="geno_test.tfam"), error=function(err){return(paste(err))})

data(geno_test.tped)
write.table(geno_test.tped, file = "geno_test.tped", row.names=FALSE, 
            col.names=FALSE, quote=FALSE, sep="\t")
tryCatch(UploadFile(local.file.name="geno_test.tped"), error=function(err){return(paste(err))})
@
}

\subsection{PLINK Conversion}

\noindent {\ttfamily PLINKConversion(file.list="", file.path="", output.type="--recode", job.name=NULL, shared.username=NULL, print.curl=FALSE, version="plink-1.07u1", suppress.Warnings=FALSE)} \\

\noindent This function converts the standard PLINK file formats (Regular (ped/map), Transposed (tped/tfam), and Binary (bed/bim/fam)) to various other PLINK file formats.

{\fs
<<PLINKConversionR, echo=TRUE>>=
myJobPLINKCT <- PLINKConversion(file.list=list("geno_test.tfam","geno_test.tped"), job.name="PCT", 
                                out.basename="plinkout")
@

<<PLINKwait, echo=FALSE>>=
Wait(myJobPLINKCT$id, minWait, maxWait)
@

<<PLINKConversionT, echo=TRUE>>=
myJobPLINKCR <- PLINKConversion(file.list=list("plinkout.map","plinkout.ped"),
                                file.path=paste("analyses/", myJobPLINKCT$name, sep=""),
                                output.type="--recode --transpose", job.name="PCR")
@
}

\noindent There are many output files possible, \url{http://pngu.mgh.harvard.edu/~purcell/plink/reference.shtml#output}

\subsection{PLINK}

\noindent {\ttfamily PLINK(file.list="", file.path="", job.name=NULL, association.method="--assoc", no.sex=TRUE, args=NULL, print.curl=FALSE, multi.adjust=TRUE, version="plink-1.07u1", shared.username=NULL, suppress.Warnings=FALSE)} \\

\noindent {\ttfamily PLINK} is an open-source whole genome association analysis toolset, designed to perform a range of basic, large-scale analyses in a computationally efficient manner, check \url{http://pngu.mgh.harvard.edu/~purcell/plink/} for details.
...
myJobPLINKR <- PLINK(file.list=list("plinkout.map","plinkout.ped"),
                     file.path=paste("analyses/", myJobPLINKCT$name, sep=""), job.name="PLINKR")
@
}
...
\subsection{FaST-LMM (Factored Spectrally Transformed Linear Mixed Models)}

\noindent {\ttfamily FaST\_LMM(input.file.list="", ALL.file.path="", print.curl=FALSE, sim.file.list=NULL, pheno.file.name=NULL, mpheno=1, args=NULL, covar.file.name=NULL, job.name=NULL, version="FaST-LMM-1.09u1", shared.username=NULL, suppress.Warnings=FALSE)} \\

\noindent {\ttfamily FaST-LMM (Factored Spectrally Transformed Linear Mixed Models)} is a program for performing genome-wide association studies (GWAS) on large data sets.  FaST-LMM is described more fully at \url{http://www.nature.com/nmeth/journal/v8/n10/abs/nmeth.1681.html}, and also at \url{http://fastlmm.codeplex.com/}
...
myJobFaST_LMMR <- FaST_LMM(input.file.list=list("plinkout.map","plinkout.ped"),
                           ALL.file.path=paste("analyses/", myJobPLINKCT$name, sep=""), 
                           job.name="FaST_LMMR")
@
}

\noindent Not all information on the FaST-LMM model is here, see the FaST-LMM website \url{http://fastlmm.codeplex.com/}, or the FaST-LMM manual for more information.

\section{Creating workflows}

\noindent Finally, each of these steps can be combined to generate multi-step analyses. This has the benefit of reducing errors that can occur when manually running each application and, more importantly, ensures that results are reproducible. In the following example, a user starts with unaligned sequences on her or his local computer and ends with aligned sequences and a phylogenetic tree with all applications running on the iPlant servers. \\

\subsection{Workflow One}

\noindent This first workflow takes an amino acid fasta file, uses {\ttfamily MUSCLE} to get a {\ttfamily PHYLIP\_PARS} alignment type.  A couple things about this alignment; it is only available from {\ttfamily MUSCLE} and this alignment is very specific to the {\ttfamily PHYLIP 3.69} model.  The {\ttfamily PHYLIP} model then produces a tree.  Note: this is the only way to do this workflow.

{\fs
<<workflow1, echo=TRUE>>=
myJobW1MP <- Muscle("PROTEIN.fasta", aln.filetype="PHYLIP_PARS", job.name="muscleWORKFLOW1protein")
Wait(myJobW1MP$id, minWait, maxWait)
myJobW1PPP <- PHYLIP_Pars("phylip_pars.aln", file.path=paste("analyses/",myJobW1MP$name,
                          sep=""), type="PROTEIN", job.name="phylipWORKFLOW1protein")
Wait(myJobW1PPP$id, minWait, maxWait)
RetrieveJob(myJobW1PPP$id, c("outtree.nwk"))
read.tree(paste(getwd(), myJobW1PPP$name, "outtree.nwk", sep="/")) -> Tree
@
}

\begin{center}
\setkeys{Gin}{width=0.59\textwidth}
{\fs
<<PHYLIPtreePLOT, echo=FALSE, fig=TRUE, eps = FALSE, height=6, width=6>>=
plot(Tree)
@
}
\end{center}

\subsection{Workflow Two}

\noindent The second workflow takes the same amino acid fasta file and this time it uses {\ttfamily Mafft} to get a {\ttfamily FASTA} alignment type.  {\ttfamily MUSCLE} also can output a fasta alignment.  The {\ttfamily FastTree} model is then used to make the tree.

{\fs
<<workflow2, echo=TRUE>>=
myJobW2CWD <- Mafft("PROTEIN.fasta", type="PROTEIN", job.name="mafftPROTEINfasta")
Wait(myJobW2CWD$id, minWait, maxWait)
myJobW2FaD <- Fasttree("mafft.fa", type="PROTEIN", file.path=paste("analyses/",myJobW2CWD$name, 
                       sep=""), job.name="fasttreeCLUSTALWfasta")
Wait(myJobW2FaD$id, minWait, maxWait)
RetrieveJob(myJobW2FaD$id, c("fasttree.nwk"))
read.tree(paste(getwd(), myJobW2FaD$name, "fasttree.nwk", sep="/")) -> Tree
@
}
...
\setkeys{Gin}{width=0.59\textwidth}
{\fs
<<FASTTREE1treePLOT, echo=FALSE, fig=TRUE, eps = FALSE, height=6, width=6>>=
plot(Tree)
@
}
\end{center}

\subsection{Workflow Three}

\noindent The third workflow is again dealing with {\ttfamily FastTree}.  {\ttfamily FastTree} can take either a FASTA alignment or a phylip interleaved alignment as inputs.  Now {\ttfamily ClustalW} takes a nucleotide fasta file to get a {\ttfamily PHYLIP INTERLEAVED} alignment type.  {\ttfamily MUSCLE} also can output that alignment.  The {\ttfamily FastTree} model is then used to make the tree.

{\fs
<<workflow3, echo=TRUE>>=
myJobW3MuP <- ClustalW("DNA.fasta", job.name="clustalwDNAfasta")
Wait(myJobW3MuP$id, minWait, maxWait)
myJobW3FaP <- Fasttree("clustalw2.fa", file.path=paste("analyses/",myJobW3MuP$name, sep=""), 
                        job.name="fasttreeMUSCLEdna")
Wait(myJobW3FaP$id, minWait, maxWait)
RetrieveJob(myJobW3FaP$id, c("fasttree.nwk"))
read.tree(paste(getwd(), myJobW3FaP$name, "fasttree.nwk", sep="/")) -> Tree
@
}
...
\setkeys{Gin}{width=0.59\textwidth}
{\fs
<<FASTTREE2treePLOT, echo=FALSE, fig=TRUE, eps = FALSE, height=6, width=6>>=
plot(Tree)
@
}
\end{center}

\subsection{Workflow Four}

\noindent The fourth workflow is using {\ttfamily RAxML}.  {\ttfamily MUSCLE} takes a nucleotide fasta file to get a {\ttfamily PHYLIP INTERLEAVED} alignment type.  {\ttfamily ClustalW} also can output that alignment.  The {\ttfamily RAxML} model is then used to make the tree.

{\fs
<<workflow4, echo=TRUE>>=
myJobW4MuP <- Muscle("DNA.fasta", aln.filetype="PHYLIP_INT", job.name="muscleWORKFLOW4dna")
Wait(myJobW4MuP$id, minWait, maxWait)
myJobW4RP <- RAxML("phylip_interleaved.aln", file.path=paste("analyses/",myJobW4MuP$name, sep=""), 
                    job.name="raxmlWORKFLOW4dna")
Wait(myJobW4RP$id, minWait, maxWait)
RetrieveJob(myJobW4RP$id, c("RAxML_bestTree.nwk"))
read.tree(paste(getwd(), myJobW4RP$name, "RAxML_bestTree.nwk", sep="/")) -> Tree
@
}
...
\setkeys{Gin}{width=0.59\textwidth}
{\fs
<<RAxMLtreePLOT, echo=FALSE, fig=TRUE, eps = FALSE, height=6, width=6>>=
plot(Tree)
@
}
\end{center}

<<last, echo=FALSE, eval=TRUE>>=
tryCatch(DeleteDir("hello"), error=function(err){return(paste(err))})
tryCatch(DeleteFile("DNA.fasta"), error=function(err){return(paste(err))})
tryCatch(DeleteFile("PROTEIN.fasta"), error=function(err){return(paste(err))})
tryCatch(DeleteFile("geno_test.tfam"), error=function(err){return(paste(err))})
tryCatch(DeleteFile("geno_test.tped"), error=function(err){return(paste(err))})
tryCatch(DeleteJob(ALL = TRUE), error=function(err){return(paste(err))})
@
